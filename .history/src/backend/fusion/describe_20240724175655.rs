...
mod tests {
    ...
    #[tokio::test]
    async fn test_percentile_25() {
        let df = create_test_dataframe();
        let percentile_df = percentile(df, 0.25).unwrap();

        let result = percentile_df.collect().await.unwrap();
        let percentile_25_value = result[0].column(0).as_any().downcast_ref::<Float64Array>().unwrap().value(0);
        assert_eq!(percentile_25_value, 1.25);  // 25th percentile of [1.0, 2.0, 3.0] is 1.5

        let percentile_25_value = result[0].column(1).as_any().downcast_ref::<Float64Array>().unwrap().value(0);
        assert_eq!(percentile_25_value, 4.5);  // 25th percentile of [4, 5, 6] is 4.5
    }
}