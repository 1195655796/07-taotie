...
async fn test_percentile() {
    ...
    // Here we call the percentile function directly to test it
    let percentile_df = percentile(describer.transformed.clone(), 0.25).unwrap();

    // Collect the result to inspect it
    let result = percentile_df.collect().await.unwrap();

    // Check the number of rows and columns in the result
    assert_eq!(result.len(), 1);  // should have one row for the percentile
    assert_eq!(result[0].num_columns(), 3);  // only numeric columns should be included

    // Verify that the values in the result are as expected
    // Since this is a test, we should calculate the expected values
    let expected_float_percentile = 1.75;  // 25th percentile of [1.0, 2.0, 3.0, 4.0]
    let expected_int_percentile = 4.75;  // 25th percentile of [4, 5, 6, 7]

    let columns = result[0].columns();
    let float_col = columns[0].as_any().downcast_ref::<Float64Array>().unwrap();
    let int_col = columns[1].as_any().downcast_ref::<Float64Array>().unwrap();

    assert_eq!(float_col.value(0), expected_float_percentile);
    assert_eq!(int_col.value(0), expected_int_percentile);
}
