...
fn percentile(df: DataFrame, p: f64) -> anyhow::Result<DataFrame> {
    let fields = df.schema().fields().iter();
    let ret = fields
        .filter(|f| f.data_type().is_numeric())
        .fold(None, |acc, field| {
            let sorted = df
                .clone()
                .sort(vec![col(field.name()).sort(true, false)])
                .unwrap();
            let count = df
                .clone()
                .aggregate(vec![], vec![count(col(field.name()))])
                .unwrap();
            let count_value = count
                ...
                .unwrap()
                .value(0);
            let percentile_index = (p * (count_value as f64 - 1.0)).ceil() as usize;

            let percentile_value = sorted
                .clone()
                .limit(percentile_index + 1)
                .unwrap()
                .select(vec![col(field.name())])
                .unwrap()
                .collect()
                .unwrap()
                .column(0)
                ...
                .unwrap()
                .value(percentile_index);

            let percentile_df = DataFrame::new(vec![vec![
                field.name().to_string(),
                percentile_value.to_string(),
            ]
            .into()]);

            match acc {
                Some(acc) => Some(acc.union(percentile_df).unwrap()),
                None => Some(percentile_df),
            }
        });

    ret.ok_or_else(|| anyhow::anyhow!("No numeric fields found"))
}
