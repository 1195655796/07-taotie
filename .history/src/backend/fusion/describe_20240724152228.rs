    ...
    async fn do_describe(&self) -> anyhow::Result<DataFrame> {
        let df: Option<DataFrame> = self.methods.iter().fold(None, |acc, method| {
            ...
            let stat_df = match method {
                ...
                DescribeMethod::Percentile(25) => percentile(df, 0.25).unwrap(),
                DescribeMethod::Percentile(50) => percentile(df, 0.5).unwrap(),
                DescribeMethod::Percentile(75) => percentile(df, 0.75).unwrap(),
            };
