...
impl DescribeDataFrame {
    ...
    fn calculate_quantile(df: &DataFrame, quantile: f64) -> DataFusionResult<DataFrame> {
        let original_schema_fields = df.schema().fields().iter();
        let quantile_exprs: Vec<_> = original_schema_fields
            .filter(|f| matches!(f.data_type(), DataType::Float64 | DataType::Int64))
            .map(|f| {
                let col_name = f.name();
                let quantile_expr = Expr::ScalarFunction {
                    fun: datafusion::scalar::functions::Quantile { quantile },
                    args: vec![col(col_name)],
                };
                quantile_expr.alias(col_name)
            })
            .collect();

        df.clone().aggregate(vec![], quantile_exprs)
    }

    fn percentile0_25(&self) -> DataFusionResult<DataFrame> {
        Self::calculate_quantile(&self.df, 0.25)
    }
}
